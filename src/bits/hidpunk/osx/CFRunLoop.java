...
class CFRunLoop {
    ...
    public synchronized void wakeup() throws CFException {
        ...
        runLoopWakeUp(mLoop.getPointer());
    }
        ...
        try{
            while(true) {
                synchronized(this) {
                    ...
                    //System.out.println("Running with: " + mSourceList.size() + " sources");
                }
            ...
            try{
                ...
                case ADD_SOURCES:
                    for(CFLoopSource s: (CFLoopSource[])ev.getRefs()) {
                        if(mSourceList.add(s)) {
                            runLoopAddSource(mLoop.getPointer(), s.getPointer());
                            s.ref();
                        }
                    }
                    break;
                ...
                case ADD_TIMERS:
                    for(CFLoopTimer t: (CFLoopTimer[])ev.getRefs()) {
                        if(mTimerList.add(t)) {
                            runLoopAddTimer(mLoop.getPointer(), t.getPointer());
                            t.ref();
                        }
                    }
                    break;
                ...
                case REMOVE_SOURCES:
                    for(CFLoopSource s: (CFLoopSource[])ev.getRefs()) {
                        if(mSourceList.remove(s)) {
                            runLoopRemoveSource(mLoop.getPointer(), s.getPointer());
                            s.deref();
                        }
                    }
                    break;
                ...
                case REMOVE_TIMERS:
                    for(CFLoopTimer t: (CFLoopTimer[])ev.getRefs()) {
                        if(mTimerList.remove(t)) {
                            runLoopRemoveTimer(mLoop.getPointer(), t.getPointer());
                            t.deref();
                        }
                    }
                    break;
    ...
    private void releaseLoop() {
        ...
        try{
            for(CFRef r: mSourceList) {
                runLoopRemoveSource(loop.getPointer(), r.getPointer());
            }
            
            for(CFRef r: mTimerList) {
                runLoopRemoveTimer(loop.getPointer(), r.getPointer());
            }
        }catch(Exception ex) {}
