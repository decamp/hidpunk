...
class NotificationHandler extends CStruct {
    ...
    public static NotificationHandler createMatchHandler
    (OsxHidManager manager, HidMatchListener listener)
    throws HidException
    {
        long structPtr = 0;
        boolean abort = true;
    ...
    public static NotificationHandler createTerminationHandler
    (HidTerminationListener listener, OsxHidDevice device)
    throws HidException
    {
        long structPtr = 0;
        boolean abort = true;
    ...
    (long structPtr, OsxHidManager manager, HidMatchListener listener) 
    throws HidException
    {
        super(structPtr);
        mManager = manager;
    ...
    private NotificationHandler
    (long structPtr, HidTerminationListener listener, OsxHidDevice device)
    throws HidException
    {
        super(structPtr);
        mManager = null;
    ...
    synchronized List<IODevice> armFound(CFNotificationSource source, OsxHidMatcher matcher) throws HidException {
        ...
        long iterPtr = addFoundNotification(getPointer(), source.getPortPointer(), matcher.getPointer());
        if(iterPtr == 0)
    ...
    synchronized boolean armRemoval(CFNotificationSource source) throws HidException {
        ...
        mArmed = true;
        return addRemovedNotification(getPointer(), source.getPortPointer(), mDevice.getIOPointer());
    }
    ...
    private void processMatch(int device) {
        ...
        try{
            ...
            OsxHidDevice dev = OsxHidDevice.constructDevice(mManager, iodev);
