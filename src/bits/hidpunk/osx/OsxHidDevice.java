...
class OsxHidDevice implements HidDevice {
    ...
    static OsxHidDevice constructDevice(OsxHidManager manager, IODevice devObj) throws HidException {
        CFRef hidDict = null;
        CFRef usbDict = null;
        long ptr;
        
        try{
            ptr = getHidDictionary(devObj.getPointer());
            hidDict = new CFRef(ptr);
            ptr = getUsbDictionary(devObj.getPointer());
            usbDict = new CFRef(ptr);

            ByteBuffer buf = ByteBuffer.allocateDirect(1056);
            buf.order(ByteOrder.nativeOrder());
            queryDeviceInfo(devObj.getPointer(), hidDict.getPointer(), usbDict.getPointer(), buf);
    ...
    public OsxHidElement getElement(int idx) {
        return mElements[idx];
    }
    
    public OsxHidElement[] getElements() {
    ...
    public OsxHidInterface openInterface() throws HidException {
        long ptr = openInterface(mDevObj.getPointer());
        if(ptr == 0)
