 ...
 * CFLoopSource object, mSource.  When mSource is freed, it will call deref()
 * on a special object, "SuperRef", which will actually dereference the 
 * OsxNotificationPort.  What this accomplishes is that, as long as anything 
 * owns a reference to either the OsxNotificationPort or the CFLoopSource,
 * they will both be valid.  However, the CFLoopSource is then guaranteed to be
 * free itself before the notification port, ensuring that a run loop thread
 * is not accessing the notification port while a different thread disposes
 * the port.
 * 
