...
/** 
 * HidValueListener specifies the methods used to received HID element 
 * values from a HidEventSource.  Unlike the <i>HidEventListener</i> interface,
 * this interface does not receive full information about each element, but 
 * collects only the value (the <i>HidEvent.mValue</i> field).  This is more
 * efficient than using HidEventListener, but does not provide timing 
 * information or any HID element "long value" (the <i>HidEvent.mLongValue</i>).
 * 
...
public interface HidValueListener {
     ...
     * <p>
     * The number of integer values held in the ByteBuffer may be inferred
     * by the ByteBuffer's limit.  For HidEventSources that query multiple
     * elements, the ByteBuffer will always contain a value for each element,
     * whether or not the element has received new data.  The order of the values
     * is determined by the order of HidElement objects used to create the
     * HidEventSource. 
     * <p>
     * This method does
     * not provide any means of determining which elements have or have not
     * produced new values.  If this information is required, you must either
     * create HidEventSources for each element, or use a HidEventListener, which
     * provides a "stale" flag for each element that differentiates new values
     * from old.  This method will not be called unless at least one element
     * has been given a new value.
     * 
