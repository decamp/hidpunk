...
public class SpaceNavigator extends AbstractHidpunkDriver<SpaceNavigatorListener> {
    ...
    protected void interfaceOpened( HidInterface inter, SpaceNavigatorListener listener ) throws HidException {
        ...
        for( HidElement el : mDevice.elements() )
            stack.add( el );
        ...
        while( !stack.isEmpty() ) {
            ...
            if( el.childCount() > 0 ) {
                for( HidElement c : el.children() ) {
                    stack.push( c );
                }
            } else {
                int page = el.usagePage();
                int use = el.usage();
        ...
        sources.add( inter.createPollingSource( 50000L, 8, new PuckAdapter( listener ), puckElements ) );
        // sources.add(inter.newAsyncSource(1,new PuckAdapter(listener),
        // puckElements));
        ...
        for( int i = 0; i < buttonElements.length; i++ ) {
            ...
            sources.add( inter.createAsyncSource( 8, new ButtonAdapter( listener, i ), buttonElements[i] ) );
        }
