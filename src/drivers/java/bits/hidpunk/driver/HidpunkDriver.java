...
public interface HidpunkDriver<L> extends EventSource<L> {
    ...
    /**
     * Starts device using specified threading strategy for passing device
     * events.
     * 
     * @param threading One of the threading constants that 
     *                  specifies threading strategy.
     * @return true if successful, false if device already running.
     */
     ...
     * method will override any calls made to addListener(),
     * addListenerWeakly() and removeListener(), although those listeners are
     * still stored and may be used if the device is restarted later with a
